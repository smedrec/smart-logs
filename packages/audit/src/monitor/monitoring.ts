...
export class MonitoringService {
	...
	constructor(config: MonitoringConfig, metricsCollector?: MetricsCollector, logger?: any) {
		...
		this.alertingService = new AlertingService(config, metricsCollector, logger)
	...
	async processEvent(event: AuditLogEvent): Promise<void> {
		...
		for (const alert of detectionResult.alerts) {
			await this.alertingService.generateAlert(alert)
		}
	...
	async getAlertStatistics(): Promise<AlertStatistics> {
		return await this.alertingService.getAlertStatistics()
	}

	async numberOfActiveAlerts(): Promise<number> {
		return await this.alertingService.numberOfActiveAlerts()
	}
