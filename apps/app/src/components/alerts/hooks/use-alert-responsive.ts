...
export interface AlertBreakpointConfig {
	sm: number
	md: number
	lg: number
	xl: number
...
const ALERT_BREAKPOINTS: AlertBreakpointConfig = {
	sm: 640,
	md: 768,
	lg: 1024,
	xl: 1280,
...
export type AlertBreakpoint = keyof AlertBreakpointConfig

/**
 * Hook for responsive design utilities specific to alert components
 */
export function useAlertResponsive(breakpoints: AlertBreakpointConfig = ALERT_BREAKPOINTS) {
	...
	const getCurrentBreakpoint = (): AlertBreakpoint => {
	...
	const isBreakpoint = (breakpoint: AlertBreakpoint): boolean => {
...
export function useAlertResponsiveTable() {
	const { isMobile, isTablet, currentBreakpoint } = useAlertResponsive()
...
export function useAlertResponsiveGrid() {
	const { currentBreakpoint, windowSize, alertColumnsCount } = useAlertResponsive()

	const getGridColumns = (config?: Partial<Record<AlertBreakpoint, number>>) => {
		if (config) {
			const breakpointOrder: AlertBreakpoint[] = ['2xl', 'xl', 'lg', 'md', 'sm']
	...
	const getGridClasses = (config: Partial<Record<AlertBreakpoint, string>>) => {
		const classes = []
...
export function useAlertTouchFriendly() {
	const { isMobile } = useAlertResponsive()
 ...
 */
export function useAlertDashboardLayout() {
	const { isMobile, isTablet, isDesktop, alertSpacing } = useAlertResponsive()
