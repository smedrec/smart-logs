...
interface ComplianceHistoryManagerProps {
	className?: string
	maxHistoryItems?: number
}

export function ComplianceHistoryManager({
	className,
	maxHistoryItems = 10,
}: ComplianceHistoryManagerProps) {
	...
	useEffect(() => {
		const updateHistory = () => {
			...
			const complianceHistory = history
				.filter((entry) => entry.url.includes('/compliance'))
