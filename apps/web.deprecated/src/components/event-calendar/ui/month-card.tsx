...
import type { Events, YearViewConfig } from '@/types/event'
		...
		return (
			<div className="grid grid-cols-7 gap-1 text-center text-xs">
				{['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
					<div key={i} className="text-muted-foreground mb-1 text-[10px] font-medium">
						{day}
					</div>
				))}
				{daysInMonth.map((day) => {
				...
				<div className="mb-3 flex items-center justify-between">
					<Tooltip>
						<TooltipTrigger asChild>
							...
							>
								{yearViewConfig.showMonthLabels && <span>{format(month, 'MMMM')}</span>}
							</Button>
						</TooltipTrigger>
						<TooltipContent side="top">View {format(month, 'MMMM yyyy')}</TooltipContent>
					</Tooltip>
					<Tooltip>
						<TooltipTrigger asChild>
							<Button
						...
						<TooltipContent side="top">Add event in {format(month, 'MMMM')}</TooltipContent>
					</Tooltip>
				</div>

				<MonthDaysGrid month={month} eventsByDate={eventsByDate} onDateClick={onDateClick} />
									...
									return (
										...
										>
											<span className="flex items-center text-white">
												<span className="mr-1 font-medium">{format(new Date(dateKey), 'd')}</span>
												{event.title}
											</span>
										</button>
									)
