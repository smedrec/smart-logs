...
import { format, getDate, getDaysInMonth, getMonth, getYear } from 'date-fns'
...
export function SearchDayPicker({
	...
	const filteredDays = useMemo(() => {
		...
		return daysInMonth.filter(
			(day) => day.searchableText.includes(searchTerm) || day.day.toString().includes(searchTerm)
		)
							...
							<AnimatePresence mode="wait">
								...
								>
									<div className="flex items-center gap-2">
										<span className="text-muted-foreground italic">{selectedDay.dayName}</span>
										<span>
											{selectedDay.day}
						...
						<CommandGroup>
							<ScrollArea className="h-[200px]">
								{filteredDays.map((day) => (
									...
									>
										<div className="flex items-center gap-2">
											<span className="text-muted-foreground italic">{day.dayName}</span>
											<span>
												{day.day}
										...
										<Check
											className={cn(
												'h-4 w-4',
												selectedDay.day === day.day ? 'opacity-100' : 'opacity-0'
											)}
										/>
									</CommandItem>
								))}
