...
export function SearchMonthPicker({
	...
	const [selectedMonthChange, setSelectedMonthChanged] = useState<boolean>(false)
					...
					>
						<div className="flex items-center">
							...
							<AnimatePresence mode="wait">
								...
								>
									<span className="truncate">{selectedMonth?.label || placeholder}</span>
								</motion.div>
							</AnimatePresence>
						</div>
						...
						<CommandGroup>
							<ScrollArea className="h-[200px]">
								{filteredMonths.map((m) => (
									<CommandItem key={m.value} value={m.value} onSelect={handleMonthChange}>
										<Check
											className={cn(
												'mr-2 h-4 w-4',
												month === parseInt(m.value) ? 'opacity-100' : 'opacity-0'
											)}
										/>
										<span className="flex-1">{m.label}</span>
										<span className="text-muted-foreground mr-2 text-sm">{m.shortLabel}</span>
									</CommandItem>
								))}
