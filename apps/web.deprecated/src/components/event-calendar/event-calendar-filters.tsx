...
export const EventCalendarFilters = () => {
	...
	const [filters, setFilters] = useQueryStates({
		...
		colors: parseAsArrayOf(parseAsString).withDefault([]).withOptions({ shallow: false }),
	...
	const toggleArrayFilter = (key: keyof typeof filters, value: string) => {
		if (key === 'dateStart' || key === 'dateEnd' || key === 'search' || key === 'isRepeating')
			return
		...
		<div className="flex flex-col space-y-2 border-b px-4 pt-2 pb-2">
			<div className="flex flex-wrap items-center justify-center gap-3 sm:justify-start">
				...
				<Popover>
					...
					<PopoverContent className="w-64 p-4">
						<div className="space-y-3">
							<h4 className="text-muted-foreground text-sm font-medium">Select Categories</h4>
				...
				<Popover>
					...
					<PopoverContent className="w-64 p-4">
						<div className="space-y-3">
							<h4 className="text-muted-foreground text-sm font-medium">Select Colors</h4>
									...
									return (
										<div key={color.value} className="flex items-center space-x-3">
											<Checkbox
												...
												onCheckedChange={() => toggleArrayFilter('colors', color.value)}
											/>
				...
				{filters.isRepeating === 'repeating' && (
					<Popover>
						<PopoverTrigger asChild>
							<Button
								variant={filters.repeatingTypes.length > 0 ? 'default' : 'outline'}
								size="sm"
								className="h-9 gap-2 px-4 text-sm font-medium transition-all"
							>
						...
						<PopoverContent className="w-52 p-4">
							<div className="space-y-3">
								<h4 className="text-muted-foreground text-sm font-medium">Repeat Frequency</h4>
								<div className="space-y-3">
									{['daily', 'weekly', 'monthly'].map((type) => (
										<div key={type} className="flex items-center space-x-3">
											<Checkbox
												...
												onCheckedChange={() => toggleArrayFilter('repeatingTypes', type)}
											/>
											<Label
												htmlFor={`repeat-${type}`}
							...
							{filters.colors.map((colorValue) => {
								...
								return (
									...
									>
										...
										/>
										<span className="text-xs font-medium">{color?.label || colorValue}</span>
										<button
