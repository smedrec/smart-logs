		...
		<div className="grid grid-cols-1 gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-1">
			<Card className="h-fit">
				<CardHeader>
					...
					<CardDescription>How your event will appear in the calendar</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-4">
										...
										>
											<span
												className={cn(
													...
													hasEvent ? 'text-primary font-bold' : 'text-muted-foreground'
												)}
											>
												{day}
											</span>
						...
						{watchedValues.title && (
							<div>
								...
								>
									<div className="font-medium" style={{ color: watchedValues.color }}>
										{watchedValues.title}
									</div>
									<div className="text-muted-foreground mt-1 flex items-center text-xs">
										...
										<span>
											...
											<span className="ml-1">
												({calculateDuration(watchedValues.startTime, watchedValues.endTime)})
											</span>
										</span>
									</div>
								</div>
					...
					<div className="space-y-4">
						{watchedValues.description && (
							<div>
								...
								{watchedValues.description && (
									<p className="text-muted-foreground mt-2 text-sm">{watchedValues.description}</p>
								)}
							</div>
						)}
						<Separator />
									...
									<p className="text-muted-foreground text-sm">
										{watchedValues.startTime && watchedValues.endTime ? (
											<>
												...
												{formatTimeDisplay(watchedValues.endTime, timeFormat)}
												<span className="ml-2">
													({calculateDuration(watchedValues.startTime, watchedValues.endTime)} Hour
													)
												</span>
											</>
										) : (
							...
							{watchedValues.category && (
								<div className="flex items-start gap-3">
									...
									<div>
										...
										<p className="text-muted-foreground mt-1 text-sm">Category</p>
									</div>
								</div>
							)}
						</div>
