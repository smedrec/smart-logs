...
export function EventCalendarTabs({
	...
	const [hoveredMobileTabIndex, setHoveredMobileTabIndex] = useState<number | null>(null)
	const [, startTransition] = useTransition()

	const { daysCount: storeDaysCount, setDaysCount: setStoreDaysCount } = useEventCalendarStore()
	...
	const visibleTabs = tabsConfig.filter((tab) => !disabledViews.includes(tab.value))
	const selectedTabIndex = visibleTabs.findIndex((tab) => tab.value === viewType)
	...
	const primarySelectedTabIndex = primaryTabs.findIndex((tab) => tab.value === viewType)
	const isSecondaryTabActive = secondaryTabs.some((tab) => tab.value === viewType)

	const navRect = navRef.current?.getBoundingClientRect()
	const mobileNavRect = mobileNavRef.current?.getBoundingClientRect()
	const selectedDesktopRect = desktopButtonRefs.current[selectedTabIndex]?.getBoundingClientRect()
								...
								<DropdownMenuTrigger asChild>
									<button
										...
										className={cn(
											...
											disabledViews.includes(tab.value) && 'cursor-not-allowed opacity-50'
										)}
						...
						<DropdownMenuContent align="end">
							{secondaryTabs.map((tab) => (
								<DropdownMenuItem
									...
									disabled={disabledViews.includes(tab.value)}
									className={cn('cursor-pointer', viewType === tab.value && 'bg-muted font-medium')}
								>
									{tab.label}
								</DropdownMenuItem>
							))}
