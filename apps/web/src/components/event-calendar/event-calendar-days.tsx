...
export function EventCalendarDays({ events, currentDate, daysCount = 16 }: CalendarDayViewProps) {
	const { timeFormat, locale, firstDayOfWeek, viewSettings, openEventDialog, openQuickAddDialog } =
	...
	const { weekDays, todayIndex } = useWeekDays(currentDate, daysCount, localeObj)

	const { singleDayEvents, multiDayEvents } = useFilteredEvents(events, weekDays)
	const eventsPositions = useEventPositions(singleDayEvents, weekDays, HOUR_HEIGHT)
	...
	const totalMultiDayRows =
		multiDayEventRows.length > 0 ? Math.max(...multiDayEventRows.map((r) => r.row)) + 1 : 1
	...
	}, [currentDate, hoverPosition, openQuickAddDialog, viewSettings.days.enableTimeSlotClick])
	...
	return (
		<div className="flex h-full flex-col overflow-hidden border">
			...
			{multiDayEventRows.length > 0 && viewSettings.days.expandMultiDayEvents && (
