...
import type { EventPosition, Events } from '@/types/event'
...
type EventDialogTriggerProps = {
	...
	leftOffset: number
	rightOffset: number
	onClick: (event: Events, position: EventPosition, leftOffset: number, rightOffset: number) => void
}
			...
			>
				...
				>
					...
					<div className="text-xs sm:truncate">
						{formatTimeDisplay(event.startTime, '12')} - {formatTimeDisplay(event.endTime, '12')}
					</div>
