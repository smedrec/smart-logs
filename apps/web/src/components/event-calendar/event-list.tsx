...
export const EVENT_VIEW_CONFIG = {
	[CalendarViewType.DAY]: {
		...
		filterFn: (eventDate: Date, currentDate: Date) => isSameDay(eventDate, currentDate),
	},
	...
	[CalendarViewType.WEEK]: {
		...
		titleFormat: 'EEEE, d MMMM yyyy',
		filterFn: (eventDate: Date, currentDate: Date) => isSameWeek(eventDate, currentDate),
	},
	[CalendarViewType.MONTH]: {
		groupFormat: 'yyyy-MM-dd',
		titleFormat: 'EEEE, d MMMM yyyy',
		filterFn: (eventDate: Date, currentDate: Date) => isSameMonth(eventDate, currentDate),
	},
	[CalendarViewType.YEAR]: {
		groupFormat: 'yyyy-MM-dd',
		titleFormat: 'EEEE, d MMMM yyyy',
		filterFn: (eventDate: Date, currentDate: Date) => isSameYear(eventDate, currentDate),
	},
}
...
const EventSection = ({
	...
	<div className="space-y-2">
		...
		<EventGroup timeKey={timeKey} events={events} timeFormat={timeFormat} onClick={onEventClick} />
	</div>
)

export function EventsList({ events, currentDate }: EventsListProps) {
	const { timeFormat, currentView, locale, openEventDialog } = useEventCalendarStore(
		useShallow((state) => ({
	...
	const groupedEvents = useEventGrouper(filteredEvents, currentView, timeFormat, localeObj)
