...
interface BaseTimeGridProps {
	...
	daysInWeek: Date[]
	todayIndex: number
	onTimeBlockClick: (data: { date: Date; startTime: string; endTime: string }) => void
}
...
export const TimeGrid = memo((props: TimeGridProps) => {
	const { highlightToday, timeSlots, daysInWeek, todayIndex, onTimeBlockClick } = props
				...
				<div key={timeIndex} className="border-border flex h-16 border-t">
					{daysInWeek.map((day, dayIndex) => (
						<button
							...
							style={isDynamic ? { width: `${props.dayWidthPercent}%` } : undefined}
							onClick={() => {
								const startTime = format(time, 'HH:mm')
								const endTime = format(new Date(time.getTime() + 60 * 60 * 1000), 'HH:mm')
								onTimeBlockClick({
									date: day,
									startTime,
									endTime,
