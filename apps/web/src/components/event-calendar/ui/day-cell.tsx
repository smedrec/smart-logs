...
import type { Events, MonthViewConfig } from '@/types/event'
			...
			{isWithinMonth && (
				<div className="item flex flex-1 flex-col justify-center gap-1 overflow-hidden">
					{shouldRenderEvents && firstEvent && (
						...
						>
							<span className="truncate font-medium text-white">{firstEvent.title}</span>
					...
					{dayEvents.length > 1 ? (
						...
						>
							...
							<span className="hidden sm:block">{dayEvents.length - 1} more</span>
						</Button>
					) : (
						<Button
							variant="ghost"
							...
							className={cn(
								...
								isEmpty ? 'mb-2 bg-transparent !ring-0 hover:!bg-transparent' : 'h-5'
							)}
							onClick={(e) => {
								e.stopPropagation()
								onQuickAdd(date)
