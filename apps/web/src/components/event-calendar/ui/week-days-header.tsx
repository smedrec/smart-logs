...
interface WeekDayHeadersProps {
	weekNumber?: number
	daysInWeek: Date[]
	formatDate: (date: Date, formatStr: string, options?: FormatOptions) => string
	locale: Locale
...
export function WeekDayHeaders({
	...
	const reorderedDays = useMemo(() => {
		...
		return ordered.slice(firstDayOfWeek).concat(ordered.slice(0, firstDayOfWeek))
		...
		<div className={cn('flex w-full items-center justify-around', className)}>
			{showWeekNumber && (
				...
				>
					...
					<div className="text-muted-foreground text-xs sm:text-sm">{weekNumber}</div>
				</motion.div>
			)}
