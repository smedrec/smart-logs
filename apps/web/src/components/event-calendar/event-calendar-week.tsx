...
export function EventCalendarWeek({ events, currentDate }: CalendarWeekProps) {
	const { timeFormat, locale, firstDayOfWeek, viewSettings, openQuickAddDialog, openEventDialog } =
	...
	const { weekNumber, weekDays, todayIndex } = useWeekDays(currentDate, DAYS_IN_WEEK, localeObj)
	...
	}, [currentDate, hoverPosition, openQuickAddDialog, viewSettings.week.enableTimeSlotClick])
	...
	return (
		<div className="flex h-full flex-col border">
			...
			{multiDayEventRows.length > 0 && viewSettings.week.expandMultiDayEvents && (
